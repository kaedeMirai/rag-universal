...
class RetrievalEngine:
    def __init__(self, config: RetrievalConfig):
        self.config = config
        self.document_lookup_pattern = re.compile(config.document_lookup_pattern, re.IGNORECASE)
    ...
    def _load_dataset(self) -> list[ChunkRecord]:
        ...
        return [self._prepare_record(idx, item) for idx, item in enumerate(raw_dataset)]

    def _prepare_record(self, idx: int, item: dict) -> ChunkRecord:
        path = item.get("path", "")
        text = normalize_text(item.get("text", ""))
        title = Path(path).name if path else f"chunk_{idx}"
    ...
    def extract_reference_tokens(self, query: str) -> set[str]:
        return {token for token in tokenize(query) if token not in self.config.reference_stopwords}
    ...
    def dense_search(self, query: str) -> dict[int, float]:
        ...
        scores, ids = self.index.search(query_embedding, self.config.dense_top_k)
    ...
    def bm25_search(self, query: str) -> dict[int, dict[str, float]]:
        ...
        raw_scores = {
            int(row["chunk_id"]): float(row["bm25_score"])
    ...
    def search(self, query: str) -> RetrievalResult:
        ...
        for doc_id in candidate_ids:
            ...
            title_coverage = (
                len(reference_tokens.intersection(item["title_tokens"])) / len(reference_tokens)
                if reference_tokens
                else 0.0
            )
            path_coverage = (
                len(reference_tokens.intersection(item["path_tokens"])) / len(reference_tokens)
                if reference_tokens
                else 0.0
            )
            all_reference_tokens = item["title_tokens"] | item["path_tokens"]
            exact_reference_match = (
                1.0 if reference_tokens and reference_tokens.issubset(all_reference_tokens) else 0.0
            )
        ...
        return RetrievalResult(chunks=self._limit_results(ranked, intent=intent), intent=intent)

    def _limit_results(self, ranked: list[RetrievedChunk], *, intent: str) -> list[RetrievedChunk]:
        per_document_limit = (
